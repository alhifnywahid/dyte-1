...
This quickstart shows how to use Dyte's Core SDK to add live video and
audio call to your applications.

For getting started quickly, you can use our sample code. You can clone and run
a sample application from the
[Core SDK GitHub repository](https://github.com).
...
1. To install the SDK, add the Dyte Core SDK dependency into the
   `pubspec.yaml` file.
...
1. Create `https://github.com` file.
